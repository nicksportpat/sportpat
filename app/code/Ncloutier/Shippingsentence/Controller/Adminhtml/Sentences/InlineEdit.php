        ...
        if ($this->getRequest()->getParam('isAjax')) {
            ...
            if (!count($postItems)) {
                ...
                foreach (array_keys($postItems) as $modelid) {
                    /** @var \Ncloutier\Shippingsentence\Model\Sentences $model */
                    $model = $this->_objectManager->create('Ncloutier\Shippingsentence\Model\Sentences')->load($modelid);
                    try {
                        $model->setData(array_merge($model->getData(), $postItems[$modelid]));
                        $model->save();
                    } catch (\Exception $e) {
                        $messages[] = "[Sentences ID: {$modelid}]  {$e->getMessage()}";
                        $error = true;
                    }
                }
            }
