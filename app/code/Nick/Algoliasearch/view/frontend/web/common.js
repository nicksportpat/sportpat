...
requirejs(['algoliaBundle'], function(algoliaBundle) {
	algoliaBundle.$(function ($) {
		...
		window.transformHit = function (hit, price_key, helper) {
			...
			var labelPercent = false;
			var labelText = "";

			if (hit['price'] !== undefined && hit['price'][algoliaConfig.currencyCode]['default_formated'] !== hit['price'][algoliaConfig.currencyCode]['group_0_formated']) {
				hit['price'][algoliaConfig.currencyCode]['default_original_formated'] = hit['price'][algoliaConfig.currencyCode]['default_formated'];
				hit['special_price'] = hit['price'][algoliaConfig.currencyCode]['group_0_formated'];
				if(typeof hit._highlightResult.price_info !== "undefined") {
					labelPercent = hit._highlightResult.price_info.regular.value ?? hit._highlightResult.price_info.regular;
					labelText = "" + Math.round(labelPercent, 1) + "%";
				} else {
					if(hit['priceinfo']) {
						labelPercent = hit['priceinfo']['regular'];
						labelText = "" + Math.round(labelPercent, 1) + "%";
					} else {
						labelText = "";
					}
			...
			if(typeof hit['images_data'] !== 'undefined') {
				var swatches = Object.keys(hit['images_data']);

				var swValues = Object.values(hit['images_data']);
				var htmlSwatches = "<ul class='product-swatch-image-container' id='swatches-"+hit.objectID+"'>";
				var i = 0;
				swatches.forEach(s => {
					...
					if (i < 12) {
						...
						let imgValue = hit['images_data'][s];

						htmlSwatches += "<li class='product-swatch-image'><img src='" + imgValue + "' alt='" + s + "' class='swatch-image'></li>"

					}

					i += 1;
				})
				let swatchleft = parseInt(swatches.length) - 12;
				if (typeof swatchleft !== 'NaN' && swatchleft > 0) {
					htmlSwatches += "<li style='width:30px;height:30px;overflow:visible;display:inline-block;float:left;text-align:center'><span style='margin-top:3px;color:red;font-weight:500;display:block;font-size:1.8rem;padding-left:3px;'>+" + swatchleft + " </span></li>";
				}
				htmlSwatches += "</ul>";
			...
			hit['html_swatches'] = htmlSwatches;

			if(hit['rating_summary'] === "") {
				hit['rating_summary'] = 0;
			}
			...
			hit['label'] = labelText;
			hit['has_label'] = labelPercent;


			hit['brand_image'] = "<img src='/media/amasty/shopby/option_images/"+hit['brand_info']['slider_image'].replace('<mark>', '').replace('</mark>','')+"' alt='"+hit['brand']+"' style='height:32px !important;'>";

			if(hit["gender"] !== "ADULT" && hit["gender"] !== "adult") {
				hit['gender_label'] = "<div class='gender_label' ><img src='" + hit['gender_icon'] + "' style='max-width:32px;max-height:32px;' alt='" + hit["gender"] + "' /></div>";
			} else {
				hit['gender_label'] = "";
			}
			hit.addToCart = {
		...
		$('body').on('click', '#refine-toggle', function () {
			...
			if ($(this).html().trim()[0] === '+')
				...
				$(this).html('+ ' + algoliaConfig.translations.refine);
			$('#instant-search-facets-container').toggleClass('visible-sm').toggleClass('visible-xs');
		});
